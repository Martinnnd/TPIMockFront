...
export default function App() {
  ...
  return <div style={theme.tokens} className={`app map-app view-${view} era-${period.decade} ${panelOpen ? 'stories-open' : ''} ${musicOpen ? 'music-open' : ''}`}>
    ...
    <EraChrome view={view} period={period} count={visibleMemories.length} panelOpen={panelOpen} musicOpen={musicOpen} onNavigate={navigate}/>
    ...
    <div hidden={view !== 'feed'}><SocialFeed decade={period.decade} onClose={() => navigate('map')} memories={memories} following={following} onFollow={author => setFollowing(v => v.includes(author) ? v.filter(a => a !== author) : [...v, author])} selected={selected} onSelect={m => setSelectedId(m?.id ?? null)} onMap={m => { setView('map'); setSelectedId(m.id); setPanelOpen(true); setFactsOpen(false); }} onAdd={startAdding}/></div>
