...
export default function Player({ decade, expanded, onToggle }: { decade: Decade; expanded: boolean; onToggle: () => void }) {
  ...
  return <section className={`music-card ${expanded ? 'expanded' : ''}`} aria-label="Música de la época" id="music-player">
    ...
    <div className="music-compact"><div className="record-art" aria-hidden="true"><Disc3 size={32}/></div><button className="track-open" onClick={onToggle} aria-expanded={expanded} aria-label={expanded ? 'Ocultar reproductor' : `Escuchar ${track.title}`}><span className="eyebrow">RADIO NOSTALGIA · {track.year}</span><strong>{track.title}</strong><small>{track.artist}</small></button><div className="music-buttons"><button className="icon-button" aria-label="Pista anterior" onClick={() => change(-1)}><SkipBack size={15}/></button><button className="icon-button" aria-label="Pista siguiente" onClick={() => change(1)}><SkipForward size={15}/></button><button className="listen-button" onClick={onToggle} aria-label={expanded ? 'Ocultar música y detener reproducción' : 'Abrir reproductor'}><Headphones size={18}/></button></div></div>
  </section>;
}
